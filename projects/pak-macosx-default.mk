...
ME_COM_HTTP_PATH      ?= src/paks/http/http.me
ME_COM_LIB_PATH       ?= ar
ME_COM_LINK_PATH      ?= ld
ME_COM_MAN_PATH       ?= man
ME_COM_MAN2HTML_PATH  ?= man2html
ME_COM_MATRIXSSL_PATH ?= /usr/src/matrixssl
ME_COM_MPR_PATH       ?= src/paks/mpr/mpr.me
ME_COM_NANOSSL_PATH   ?= /usr/src/nanossl
ME_COM_OPENSSL_PATH   ?= /usr/src/openssl
ME_COM_OSDEP_PATH     ?= src/paks/osdep/osdep.me
ME_COM_PCRE_PATH      ?= src/paks/pcre
ME_COM_PMAKER_PATH    ?= /Applications/PackageMaker.app/Contents/MacOS/PackageMaker
...
CFLAGS                += -w
DFLAGS                +=  $(patsubst %,-D%,$(filter ME_%,$(MAKEFLAGS))) -DME_COM_EJS=$(ME_COM_EJS) -DME_COM_EST=$(ME_COM_EST) -DME_COM_MATRIXSSL=$(ME_COM_MATRIXSSL) -DME_COM_NANOSSL=$(ME_COM_NANOSSL) -DME_COM_OPENSSL=$(ME_COM_OPENSSL) -DME_COM_PCRE=$(ME_COM_PCRE) -DME_COM_SQLITE=$(ME_COM_SQLITE) -DME_COM_SSL=$(ME_COM_SSL) -DME_COM_ZLIB=$(ME_COM_ZLIB) 
IFLAGS                += "-I$(CONFIG)/inc"
...
$(CONFIG)/bin/pak: $(DEPS_34)
	...
	$(CC) -o $(CONFIG)/bin/pak -arch $(CC_ARCH) $(LDFLAGS) $(LIBPATHS) "$(CONFIG)/obj/pak.o" $(LIBPATHS_34) $(LIBS_34) $(LIBS_34) $(LIBS) 
