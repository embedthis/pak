...
//  MOB - better name than blendSpec
    ...
    private function installPak(pak: Package): Void {
        ...
        if (spec.pak.import && getPakSetting(pak, 'noimport') !== true) {
            if (export.length == 0) {
                ...
                /* UNUSED
                let path = pak.cachePath.join('doc')
                if (path.exists) {
                    files ||= []
                    files.push('doc/**')
                    export.push({ from: 'doc/', to: '${TOP}' })
                }
                */
            }
        } else {
